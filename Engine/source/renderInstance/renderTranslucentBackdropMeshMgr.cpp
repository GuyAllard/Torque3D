#include "renderTranslucentBackdropMeshMgr.h"
...
:  RenderBinManager( RenderPassManager::RIT_TranslucentBackdropMesh, 1.0f, 1.0f )
{
}
...
void RenderTranslucentBackdropMeshMgr::addElement( RenderInst *inst )
{
   ...
   if( inst->type != RenderPassManager::RIT_TranslucentBackdropMesh )
      return;
...
void RenderTranslucentBackdropMeshMgr::render( SceneRenderState *state )
{
   ...
   for( U32 j=0; j<binSize; )
   {
      ...
      if ( baseRI->type == RenderPassManager::RIT_TranslucentBackdropMesh )
      {
         ...
         while( mat->setupPass( state, sgData ) )
         {
            ...
            for( a=j; a<binSize; a++ )
            {
               ...
               if ( nextRI->type != RenderPassManager::RIT_TranslucentBackdropMesh )
                  break;
